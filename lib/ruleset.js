...
Ruleset.prototype.tryWrite = function(path, root, newData, auth) {
  ...
  (function traverse(pathParts, remainingPath, rules) {
    ...
    if (rules.hasOwnProperty('.validate') && result.validated) {
      ...
      rule = rules['.validate'];
      result.info += '/' + pathParts.join('/') + ':.validate: "' + rules['.validate'].toString() + '"\n';
    ...
    if (( nextPathPart = remainingPath.shift() )) {
      ...
      if (rules.hasOwnProperty(nextPathPart)) {
        traverse(pathParts.concat(nextPathPart), remainingPath, rules[nextPathPart]);
      } else if (rules.hasOwnProperty('$')) {
        // wildchild.
        traverse(pathParts.concat(nextPathPart), remainingPath, rules.$);
      }
      ...
      if (typeof val === 'object' && val !== null) {
        ...
        Object.keys(val).forEach(function(key) {
          ...
          if (rules.hasOwnProperty(key)) {
            traverse(pathParts.concat(key), remainingPath, rules[key]);
          } else if (rules.hasOwnProperty('$')) {
            // wildchild.
            traverse(pathParts.concat(key), remainingPath, rules.$);
          }
