...
const pathSplitter = require('./helpers').pathSplitter;
const pathMerger = require('./helpers').pathMerger;
...
Ruleset.prototype.tryRead = function(path, initialData, auth, now) {

  var state = {
    auth: auth === undefined ? null : auth,
    now: now || Date.now(),
    root: initialData.snapshot('/'),
    data: initialData.snapshot(path)
  };
...
Ruleset.prototype.tryWrite = function(path, initialData, newValue, auth, skipWrite, skipValidate, skipOnNoValue, now) {
  ...
  const newData = initialData.set(path, newValue, undefined, now);
  const result = {
    path: path,
    type: 'write',
    info: '',
    ...
    validated: true,
    data: initialData.snapshot(path),
    newData: newData.snapshot(path),
    root: initialData.snapshot('/'),
    newRoot: newData.snapshot('/')
  };

  return this._tryWrite(path, initialData, newData, result, skipWrite, skipValidate, skipOnNoValue);
};

Ruleset.prototype.tryPatch = function(path, initialData, newValues, auth, skipWrite, skipValidate, skipOnNoValue, now) {
  const pathsToTest = [];
  let newData = initialData;
  let pathResults, result;

  now = now || Date.now();

  Object.keys(newValues).forEach(function(endPath){
    var pathToNode = pathMerger(path, endPath);

    newData = newData.set(pathToNode, newValues[endPath], undefined, now);
  ...
  pathResults = pathsToTest.map(function(pathToNode) {
    ...
    return this._tryWrite(pathToNode, initialData, newData, pathResult, skipWrite, skipValidate, skipOnNoValue);
  }, this);
  ...
  result.data = initialData.snapshot(path);
  result.newData = newData.snapshot(path);
  result.root = initialData.snapshot('/');
  result.newRoot = newData.snapshot('/');
...
Ruleset.prototype._tryWrite = function(path, initialData, newData, result, skipWrite, skipValidate, skipOnNoValue) {
  ...
  (function traverse(pathParts, remainingPath, rules, wildchildren) {
    ...
    var state = Object.assign({
      ...
      root: initialData.snapshot('/'),
      data: initialData.snapshot(currentPath),
    ...
    if (( nextPathPart = remainingPath.shift() )) {
      ...
      const val = newData.snapshot(currentPath).val();
