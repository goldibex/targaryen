  ...
  switch (kind) {
    case ".indexOn":
      if (ruleType !== "string" && !Array.isArray(value)) {
        throw new RuleError(
          stack,
          `Expected .indexOn to be a string or an Array, got ${ruleType}`
        );
      }
      ...
      if (Array.isArray(value) && value.some(i => typeof i !== "string")) {
        throw new RuleError(
          stack,
          `Expected .indexOn to be an Array of string, got ${value
            .map(x => typeof x)
            .join(", ")}`
        );
      }
    ...
    case ".validate":
      if (ruleType !== "string" && ruleType !== "boolean") {
        throw new RuleError(
          stack,
          `Expected ${kind} to be a string or a boolean, got ${ruleType}`
        );
      }
...
class Ruleset {
  ...
  constructor(rulesDefinition) {
    ...
    if (!rulesDefinition.rules || Object.keys(rulesDefinition).length !== 1) {
      throw new Error(
        'Rules definition must have a single root object with property "rules"'
      );
    }
  ...
  static evaluate(options) {
    ...
    try {
      const result = options.debug
        ? rule.debug(options.state)
        : { value: rule.evaluate(options.state) };
...
class RulesetNode {
  ...
  constructor(stack, rules) {
    ...
    if (!isObject(rules)) {
      throw new RuleError(
        stack,
        `rules should be an object, got ${typeof rules}`
      );
    }
    ...
    if (wildchild.length > 1) {
      throw new RuleError(
        stack,
        "There can only be one wildchild at a given path."
      );
    }
    ...
    Object.defineProperties(this, {
      ...
      $read: {
        value:
          rules[".read"] == null
            ? null
            : parser.parse(rules[".read"].toString(), wildchildren, !isWrite)
      },
      $write: {
        value:
          rules[".write"] == null
            ? null
            : parser.parse(rules[".write"].toString(), wildchildren, isWrite)
      },
      $validate: {
        value:
          rules[".validate"] == null
            ? null
            : parser.parse(rules[".validate"].toString(), wildchildren, isWrite)
      }
    });

    // setup children rules
    const children = Object.keys(rules).filter(
    ...
    this.$wildchild = wildchildName
      ? new RulesetNode(stack.concat(wildchildName), rules[wildchildName])
      : null;
