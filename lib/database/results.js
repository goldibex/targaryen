...
class Result {
  ...
  get logs() {
    ...
    return [{ path: paths.trim(this.path), hasNoRules: true }].concat(
      this[logsKey]
    );
  }
  ...
  get info() {
    const info = [this.description, this.evaluations];
  ...
  get description() {
    const op = `Attempt to ${this.type} ${this.path} as ${JSON.stringify(
      this.auth
    )}.\n`;

    switch (this.type) {
      case "write":
        return `${op}New Value: "${JSON.stringify(
          this.newValue,
          undefined,
          2
        )}".\n`;
      ...
      case "patch":
        return `${op}Patch: "${JSON.stringify(
          this.newValue,
          undefined,
          2
        )}".\n`;
...
exports.update = function(path, data, patch, writeResults) {
  const result = new Result(path, "patch", data, {
    newDatabase: data,
    newValue: patch
  });
