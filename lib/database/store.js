...
class DataNode {
  ...
  constructor(value, now) {
    ...
    keys.filter(key => key !== ".value" && key !== ".priority").forEach(key => {
      if (!isValidKey(key)) {
        throw new Error(
          `Invalid key "${key}"; it must not include [${invalidChar.join(",")}]`
        );
      }
  ...
  static from(value, priority, now) {
    ...
    if (isServerValue(value)) {
      return new DataNode({
        ".value": convertServerValue(value, now),
        ".priority": priority
      });
    }
  ...
  $child(path) {
    return paths
      .split(path)
      .reduce((parent, key) => parent[key] || DataNode.null(), this);
  }
