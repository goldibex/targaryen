...
class Database {
  ...
  constructor(opts) {
    ...
    this.root = store.create(opts.data === undefined ? null : opts.data, {
      now: this.timestamp
    });
  ...
  update(path, patch, nowOrOptions) {
    ...
    const pathsToTest = Object.keys(patch).map(endPath =>
      paths.join(path, endPath)
    );
    const writeResults = pathsToTest.map(p =>
      this.rules.tryWrite(p, this, newDatabase, patch)
    );
