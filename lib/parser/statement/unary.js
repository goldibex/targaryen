...
class UnaryNode extends Node {
  ...
  inferType() {
    ...
    switch (this.operator) {
      case "!":
        this.assertType(argType, "boolean", {
          mustComply,
          msg: msg("boolean")
        });
        return "boolean";
      ...
      default:
        throw new ParseError(
          this,
          `Unary expressions may not contain "${this.operator}" operator.`
        );
    }
  }
  ...
  evaluateWith(state, value) {
    switch (this.operator) {
      ...
      case "-":
        this.assertType(types.from(value), "number", {
          mustComply: true,
          msg: "- only operate on number"
        });
        return -value;
