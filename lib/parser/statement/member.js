...
class MemberNode extends Node {
          ...
          args(scope, fnNode, mustComply) {
            ...
            if (!keyList.elements) {
              throw new ParseError(
                fnNode,
                "hasChildren takes 1 argument: an array of strings"
              );
            }
            ...
            if (keyList.elements.length === 0) {
              throw new ParseError(
                fnNode,
                "hasChildren got an empty array, expected some strings in there"
              );
            }

            keyList.elements.forEach(element =>
              fnNode.assertType(element.inferredType, "string", {
                mustComply,
  ...
  init(source, astNode, scope) {
    ...
    this.assertType(objectType, ["Query", "RuleDataSnapshot", "string"], {
      msg
    });
    ...
    try {
      this.property = Node.from(
        ...
        objectType === "any"
          ? scopeFactory.any()
          : scopeFactory.create(MemberNode.properties[objectType])
      );
    } catch (e) {
  ...
  inferAsStringMethod() {
    ...
    if (!this.computed) {
      this.property.inferredType = this.property.inferType(
        scopeFactory.create(MemberNode.properties.string)
      );
    }
