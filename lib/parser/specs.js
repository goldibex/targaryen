...
class Rule {
   ...
   * @return {Array<{rule: string, user: string, isValid: boolean, evaluateTo: boolean, failAtRuntime: boolean}>}
   */
  static evaluateRules(rules, users) {
    const tokens = firebase.tokens(users);

    return rules.reduce((p, spec) => p.then(results => {
...
exports.test = Rule.evaluateRules;
