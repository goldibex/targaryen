...
class DataNode {
  ...
  $walk(path, cb) {
    Object.keys(this).forEach(key => {
      const nodePath = paths.pathMerger(path, key);
   ...
   */
  $child(path) {
    return paths.pathSplitter(path).reduce(
  ...
  $set(path, value, priority, now) {
    ...
    paths.pathSplitter(path).forEach((key, i, pathParts) => {
  ...
  $remove(path, now) {
    ...
    paths.pathSplitter(path).every((key) => {
      const siblings = Object.keys(currSrc).filter(k => k !== key);
