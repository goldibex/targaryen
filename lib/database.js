...
class Database {
   ...
   */
  constructor(rules, value, auth, now) {

    now = now || Date.now();

    this.rules = ruleset.create(rules);
    this.root = store.create(value, {now: this.timestamp});
    this.auth = auth || null;
    this.timestamp = now;
   ...
   *
   * @param  {{rules: any, value: any, auth: any}} updates Database attribute to replace.
   * @return {Database}
   */
  with(updates) {
    ...
    return new Database(attr.rules, attr.value, attr.auth, attr.now);
  }
  ...
  write(path, value, priority, now) {
    ...
    const newData = this.with({value: newRoot, now});
  ...
  update(path, patch, now) {
    ...
    const pathsToTest = Object.keys(patch).map(function(endPath){
      var pathToNode = paths.pathMerger(path, endPath);
...
class RuleDataSnapshot {
   ...
   */
  child(childPath) {
    const newPath = paths.pathMerger(this[pathKey], childPath);
 ...
 *
 * @param  {Ruleset|object}  rules [description]
 * @param  {DataNode|object} data  [description]
...
exports.snapshot = function(path, value, now) {
  const data = new Database({rules: {}}, null, now);

  return data
    .with({value: data.root.$set(path, value, undefined, now)})
    .snapshot();
