...
const SECRET_PATH =
  process.env.TARGARYEN_SECRET_PATH || path.resolve("targaryen-secret.json");
let CACHED_SECRET;
...
function loadSecret(options) {
  ...
  return readFile(SECRET_PATH)
    .then(
      content => JSON.parse(content),
      () =>
        Promise.reject(
          new Error(`Failed to load ${SECRET_PATH}!
...
exports.canRead = function(path, token, options) {
  ...
  return loadSecret(options).then(secret => {
    ...
    const logURI = `${databaseURL}/${path}.json?${qs.stringify(
      secureQuery(query)
    )}`;
