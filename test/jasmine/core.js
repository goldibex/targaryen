...
describe("the targaryen Jasmine plugin", function() {
  ...
  it("should test multi write access", function() {
    ...
    expect(targaryen.users.unauthenticated).cannotPatch("/", {
      foo: { bar: 2, baz: 2 }
    });
    expect(targaryen.users.unauthenticated).cannotPatch("/", {
      "foo/bar": 1,
    ...
    expect(targaryen.users.unauthenticated).cannotPatch("/foo", {
      bar: 1,
      baz: 2
    });
    expect(targaryen.users.unauthenticated).cannotPatch("/foo", {
    ...
    expect(targaryen.users.facebook).cannotPatch("/", {
      foo: { bar: 2, baz: 2 }
    });
    expect(targaryen.users.facebook).cannotPatch("/", {
      "foo/bar": 1,
