...
describe("Rule", function() {
  describe("constructor", function() {
    ...
    it("should provide global scope when infering a computed property type", function() {
      expect(() => parser.parse("root[$foo]() == null", ["$foo"])).to.throw(
  ...
  describe("#evaluate", function() {
    it("returns the correct result of evaluating the rule given the variable scope", function() {
      fixtures.tests
        .filter(d => d.isValid && !d.failAtRuntime)
        .forEach(function(details) {
          ...
          const rule = parser.parse(
            details.rule,
            Object.keys(details.wildchildren || {})
          );

          expect(rule.evaluate(state), details.rule).to.equal(
            details.evaluateTo
          );
        });
    });
  ...
  describe("#debug", function() {
    ...
    it("should format ternary expression 1/2", function() {
      const rule = parser.parse(
        'auth.isAdmin === true ? true : root.child("open").exists()',
        []
      );
      const state = {
        root: database.snapshot("/", null),
        auth: { isAdmin: true }
      };
    ...
    it("should format ternary expression 2/2", function() {
      const rule = parser.parse(
        'auth.isAdmin === false ? root.child("open").exists() : true',
        []
      );
      const state = {
        root: database.snapshot("/", null),
