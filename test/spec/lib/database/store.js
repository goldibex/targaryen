...
describe("store", function() {
  ...
  describe("should throw when creating a node with an invalid name:", function() {
    [".", "#", "$", "[", "]"].forEach(char => {
      ...
      it(`e.g. using "${char}"`, () =>
        expect(() => store.create(data)).to.throw());
    });
  });
  ...
  describe("#$priority", function() {
    ...
    it("should return the node priority set with explicite priority", function() {
      ...
      expect(data.$set("foo", value, "something").foo.$priority()).to.equal(
        "something"
      );
    });
    ...
    it("should throw when setting invalid priority", function() {
      expect(() =>
        store.create({ ".value": 1, ".priority": { foo: "bar" } })
      ).to.throw();
  ...
  describe("#$set", function() {
    ...
    describe("should throw when the path includes an invalid character:", function() {
      [".", "#", "$", "[", "]"].forEach(char => {
        it(`e.g. using "${char}"`, () =>
          expect(() => data.$set(`a${char}c`, 1)).to.throw());
      });
    });
  });
  ...
  describe("#$remove", function() {
    ...
    describe("should throw when the path includes an invalid character:", function() {
      [".", "#", "$", "[", "]"].forEach(char => {
        it(`e.g. using "${char}"`, () =>
          expect(() => data.$remove(`/a${char}c`)).to.throw());
      });
    });
  });
