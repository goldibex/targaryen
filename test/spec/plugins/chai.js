...
describe("Chai plugin", function() {
  ...
  it("should test read access with provided query", function() {
    ...
    expect(targaryen.users.unauthenticated).cannot.read.path("/");
    expect(targaryen.users.facebook).cannot.read.path("/");
    expect(targaryen.users.facebook)
      .can.readWith({
        query: {
          orderByChild: "owner",
  ...
  it("should test multi write access", function() {
    ...
    expect(targaryen.users.unauthenticated)
      .cannot.patch({ foo: { bar: 2, baz: 2 } })
      .path("/");
    expect(targaryen.users.unauthenticated)
      .cannot.patch({ "foo/bar": 1, "foo/baz": 2 })
      .path("/");
    expect(targaryen.users.unauthenticated)
      .cannot.patch({ "foo/bar": 2, "foo/baz": 2 })
      .path("/");
    expect(targaryen.users.unauthenticated)
      .cannot.patch({ bar: 1, baz: 2 })
      .path("/foo");
    expect(targaryen.users.unauthenticated)
      .cannot.patch({ bar: 2, baz: 2 })
      .path("/foo");
    ...
    expect(targaryen.users.facebook)
      ...
      .path("/");
    expect(targaryen.users.facebook)
      .can.patch({ "foo/bar": 2, "foo/baz": 2 })
      .path("/");
    expect(targaryen.users.facebook)
      .cannot.patch({ bar: 1, baz: 2 })
      .path("/foo");
    expect(targaryen.users.facebook)
      .can.patch({ bar: 2, baz: 2 })
      .path("/foo");
  });
  ...
  it("can set operation time stamp", function() {
    ...
    expect(null)
      .can.write({ ".sv": "timestamp" }, { now: 1000 })
      .path("/foo");
    expect(null)
      .can.write({ ".sv": "timestamp" })
      .path("/foo");

    expect(null)
      .can.write(1000, { now: 1000 })
      .path("/foo");
    expect(null)
      .cannot.write(2000, { now: 1000 })
      .path("/foo");

    expect(null)
      .can.patch({ foo: { ".sv": "timestamp" } }, { now: 1000 })
      .path("/");
    expect(null)
      .can.patch({ foo: { ".sv": "timestamp" } })
      .path("/");

    expect(null)
      .can.patch({ foo: 1000, bar: 1000 }, { now: 1000 })
      .path("/");
    expect(null)
      .cannot.patch({ foo: 1000, bar: 1000 }, { now: 2000 })
      .path("/");
  });
  ...
  it("can set operation time stamp (legacy)", function() {
    ...
    expect(null)
      ...
      .path("/foo");
    expect(null)
      .cannot.write(2000, 1000)
      .path("/foo");

    expect(null)
      .can.patch({ foo: { ".sv": "timestamp" } }, 1000)
      .path("/");
    expect(null)
      .can.patch({ foo: { ".sv": "timestamp" } })
      .path("/");
    ...
    expect(null)
      .can.patch({ foo: 1000, bar: 1000 }, 1000)
      .path("/");
    expect(null)
      .cannot.patch({ foo: 1000, bar: 1000 }, 2000)
      .path("/");
  });
});
