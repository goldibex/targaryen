...
function chaiTargaryen(chai, utils) {
  ...
  chai.Assertion.addChainableMethod(
    "readAt",
    function(now) {
      const options = Object.assign({}, utils.flag(this, "operationOptions"), {
        now
      });
  ...
  chai.Assertion.addMethod("path", function(path) {
    ...
    switch (operationType) {
      case "read":
        ...
        if (positivity) {
          chai.assert(
            result.allowed === true,
            targaryen.util.unreadableError(result, 6).trim()
          );
        } else {
          chai.assert(
            result.allowed === false,
            targaryen.util.readableError(result, 6).trim()
          );
        }
    ...
    if (positivity) {
      chai.assert(
        result.allowed === true,
        targaryen.util.unwritableError(result, 6).trim()
      );
    } else {
      chai.assert(
        result.allowed === false,
        targaryen.util.writableError(result, 6).trim()
      );
    }
  });
}
